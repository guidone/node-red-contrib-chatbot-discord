...
const DiscordServer = require('../lib/platforms/discord');
...
module.exports = function(RED) {
  ...
  function DiscordBotNode(n) {
    ...
    var discordConfigs = RED.settings.functionGlobalContext.get('discord') || {};
    ...
    if (!startNode) {
      ...
      console.log(warn('Discord Bot ' + this.botname + ' will NOT be launched, environment is ' + environment));
      return;
    }
    // eslint-disable-next-line no-console
    console.log(green('Discord Bot ' + this.botname + ' will be launched, environment is ' + environment));
    ...
    if (discordConfigs[node.botname] != null) {
      var validation = validators.platform.discord(discordConfigs[node.botname]);
      if (validation != null) {
        /* eslint-disable no-console */
        console.log('');
        console.log(lcd.error('Found a Discord configuration in settings.js "' + node.botname + '", but it\'s invalid.'));
        ...
        console.log('');
        console.log(lcd.grey('Found a valid Discord configuration in settings.js: "' + node.botname + '":'));
        console.log(prettyjson.render(discordConfigs[node.botname]));
        console.log('');
        /* eslint-enable no-console */
        botConfiguration = discordConfigs[node.botname];
      }
    }
    ...
    if (node.chat == null && botConfiguration.token != null) {
      ...
      try {
        ...
        node.chat = DiscordServer.createServer({
  ...
  RED.nodes.registerType('chatbot-discord-node', DiscordBotNode, {
    credentials: {
      token: {
        type: 'text'
      }
  ...
  function DiscordInNode(config) {
    ...
    if (this.config) {
      ...
      if (node.chat) {
        ...
        nodeGlobalKey = 'discord_master_' + this.config.id.replace('.','_');
        var isMaster = false;
        ...
        node.warn('Missing or incomplete configuration in Discord Receiver');
      }
    } else {
      node.warn('Missing configuration in Discord Receiver');
    }
  ...
  RED.nodes.registerType('chatbot-discord-receive', DiscordInNode);

  function DiscordOutNode(config) {
    RED.nodes.createNode(this, config);
    var node = this;
    var global = this.context().global;
    var environment = global.environment === 'production' ? 'production' : 'development';
